...
local sparse = {"head"}
sparse[3] = "tail"
local sparseCount = count(unpack(sparse, 1, 4))
local p, q, r, s = unpack(sparse, 1, 4)
result = a .. b .. c .. ":" .. x .. y .. ":" .. sparseCount .. ":" .. tostring(q) .. ":" .. r .. ":" .. tostring(s)
`); err != nil {
		t.Fatalf("DoString() error = %v", err)
	}
	got, _ := st.GetGlobal("result")
	if got.String() != "ABC:87:4:nil:tail:nil" {
...
local missingArgOK, missingArgErr = pcall(function()
  return string.format("%s:%d", "only")
end)
result = integerFormats .. ":" .. extraIgnored .. ":" .. tostring(missingArgOK) .. ":" .. type(missingArgErr)
`); err != nil {
		t.Fatalf("DoString() error = %v", err)
	}
	got, _ := st.GetGlobal("result")
	if got.String() != "7:-3:ff:FF:11:A:kept:false:string" {
...
local clock12 = os.date("!%I:%p", 0)
local t = os.date("!*t", 0)
result = formatted .. ":" .. ordinal .. ":" .. names .. ":" .. clock12 .. ":" .. t.year .. ":" .. t.month .. ":" .. t.day .. ":" .. t.hour .. ":" .. t.min .. ":" .. t.sec .. ":" .. tostring(t.isdst)
`); err != nil {
		t.Fatalf("DoString() error = %v", err)
	}
	got, _ := st.GetGlobal("result")
	if got.String() != "1970-01-01 00:00:00:4:001:Thu:Thursday:Jan:January:12:AM:1970:1:1:0:0:0:false" {
